#!/bin/bash
# Run TASB-MSMARCO benchmark on EC2
set -e

DATA_PATH=${1:-"/data/tasb-msmarco"}
INDEX_PATH=${2:-"/tmp/tasb.idx"}
OUTPUT_LOG=${3:-"/tmp/tasb_benchmark.log"}

echo "=== TASB-MSMARCO Benchmark ===" | tee "$OUTPUT_LOG"
