#!/bin/bash
# Run MPNet-MSMARCO benchmark on EC2
set -e

DATA_PATH=${1:-"/data/mpnet-msmarco"}
INDEX_PATH=${2:-"/tmp/mpnet.idx"}
OUTPUT_LOG=${3:-"/tmp/mpnet_benchmark.log"}

echo "=== MPNet-MSMARCO Benchmark ===" | tee "$OUTPUT_LOG"
