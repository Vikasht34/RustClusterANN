...
#[tokio::main]
async fn main() {
    println!("=== SIFT 1M: 4-bit Quantization with Reranking ===\n");
    
    let data_path = "/Users/viktari/pysptag/data/sift";
    let index_path = "/tmp/sift_4bit.idx";
    
    // Build index with 4-bit quantization
    ...
    println!("Building index with 4-bit quantization...");
    let start = Instant::now();
    let mut index = SPANNIndex::new();
    index.set_metric(DistanceMetric::L2);
    index.set_quantization(QuantizationType::FourBit);
    ...
    println!("=== Index Size ===");
    println!("Posting lists (4-bit + zstd): {:.2} MB", posting_size as f32 / 1024.0 / 1024.0);
    ...
    loaded_index.set_num_heads_to_search(256);
    loaded_index.set_max_check(16384);
    ...
    // Test with different rerank factors
    println!("=== Testing different rerank factors ===\n");
    
    for rerank_factor in [5, 10, 20, 50] {
        let start = Instant::now();
        let mut total_recall = 0.0;
        
        for (i, query) in queries.iter().take(100).enumerate() {
            let results = loaded_index.search_async_with_rerank(query, 10, rerank_factor, &storage).await.unwrap();
        ...
        println!("Rerank factor {}: Recall@10={:.2}%, Latency={:.2}ms", 
            rerank_factor, avg_recall * 100.0, elapsed.as_secs_f32() * 1000.0 / 100.0);
    }
}
