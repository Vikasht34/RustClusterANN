use rustsptag::spann::{SPANNIndex, DistanceMetric};
use std::fs::File;
use std::io::{BufReader, Read};

fn read_binary_vectors(filename: &str, limit: usize) -> Vec<Vec<f32>> {
    let file = File::open(filename).unwrap();
    let mut reader = BufReader::new(file);
    
    let mut header = [0u8; 8];
    reader.read_exact(&mut header).unwrap();
    let n = u32::from_le_bytes([header[0], header[1], header[2], header[3]]) as usize;
    let d = u32::from_le_bytes([header[4], header[5], header[6], header[7]]) as usize;
    
    let n = n.min(limit);
    let mut vectors = Vec::with_capacity(n);
    let mut buffer = vec![0u8; d * 4];
    
    for _ in 0..n {
        reader.read_exact(&mut buffer).unwrap();
        let vec: Vec<f32> = (0..d)
            .map(|i| f32::from_le_bytes([
                buffer[i * 4],
                buffer[i * 4 + 1],
                buffer[i * 4 + 2],
                buffer[i * 4 + 3],
            ]))
            .collect();
        vectors.push(vec);
    }
    
    vectors
}

fn main() {
    println!("Building index with 10K vectors...");
    let base = read_binary_vectors("/Users/viktari/rustsptag/data/base.bin", 10000);
    
    let mut index = SPANNIndex::new();
    index.set_metric(DistanceMetric::InnerProduct);
    index.build(base);
    println!("Built!\n");
    
    println!("Loading 3 queries...");
    let queries = read_binary_vectors("/Users/viktari/rustsptag/data/query.bin", 3);
    
    println!("Testing for duplicates...");
    for (i, query) in queries.iter().enumerate() {
        let results = index.search(query, 10);
        let ids: Vec<_> = results.iter().map(|(id, _)| id).collect();
        let unique: std::collections::HashSet<_> = ids.iter().collect();
        
        println!("Query {}: {} results, {} unique", i, ids.len(), unique.len());
        if ids.len() != unique.len() {
            println!("  DUPLICATES FOUND: {:?}", ids);
        } else {
            println!("  No duplicates: {:?}", ids);
        }
    }
}
