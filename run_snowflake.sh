#!/bin/bash
# Run Snowflake-MSMARCO benchmark on EC2
set -e

DATA_PATH=${1:-"/data/snowflake-msmarco"}
INDEX_PATH=${2:-"/tmp/snowflake.idx"}
OUTPUT_LOG=${3:-"/tmp/snowflake_benchmark.log"}

echo "=== Snowflake-MSMARCO Benchmark ===" | tee "$OUTPUT_LOG"
